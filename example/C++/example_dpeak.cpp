...
int main()
{
    ...
    for (int i = 0; i < gridsize; i++)
    {
        ...
        {
            marginal_hi[i] += std::exp(likelihood[i*gridsize + j]);
            marginal_lo[j] += std::exp(likelihood[i*gridsize + j]);
        }
    }
    ...
    float z1 = sqrt(2) * erf_inv(2 * q1 - 1);
    ...
    float z2 = sqrt(2) * erf_inv(2 * q2 - 1);
